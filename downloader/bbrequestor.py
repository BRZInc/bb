...
class BitBucketRequestor():
    ...
    def get_all_pull_requests(self, project, repo):
        try:
            response = requests.get(
                self.__build_pull_requests_url(project, repo), {})
            pull_requests = []
            if response.OK:
                j = response.json()
                for p in j.get("values", []):
                    reviewers_ids = [revs["user"]["id"]
                                     for revs in p["reviewers"]]
                    participant_ids = [pars["user"]["id"]
                                       for pars in p["participants"]]
    ...
    def __build_repos_url(self, project):
        return urllib.parse.urljoin(
            self.url,
            "{0}{1}/{2}".format(self.__projects, project, self.__repos))

    def __build_pull_requests_url(self, project, repo):
        return urllib.parse.urljoin(
            self.url,
            "{0}{1}/{2}{3}/{4}".format(self.__projects, project, self.__repos, repo, self.__pull_requests))
